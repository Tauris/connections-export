...
- **[Hugo](https://gohugo.io)** is another static-site generator. The export is Hugo *content* — the pages, images and files — to drop into a Hugo site you already have, with your own theme and configuration; ask for the optional [starter site](#man-export-hugo-starter) and it can be viewed on its own as well.
...
**From the console.** Open an archive in the Reader, then use **Export → “Advanced — developer formats”** and choose **Export Obsidian vault…**, **Export Jekyll site…** or **Export Hugo content…**. Each opens the same [guided export](#man-export-combine) as the Archives screen, for the archive you are reading, at its **Format** step with that format chosen — so every option is there, the Hugo starter site among them, and nothing is written before a **Check**. An archive you opened from somewhere else (a dropped folder or `.zip`) is exported on its own; one from the archives folder can have others added to it. The export goes into a folder beside the archive (`…-obsidian-vault` / `…-jekyll-site` / `…-hugo-content`), and the dialog tells you exactly where, and how much of the page content stayed HTML (see [Page content](#man-export-html)). **Page content as** chooses how pages are written; leave it on *Default for the format* unless you have a reason. The advanced section is tucked away on purpose, so the everyday PDF export stays front and centre.
...
Add `--starter-site` to a Hugo export to write the [starter site](#man-export-hugo-starter) beside its content, and `--starter-layout cards` to draw its front page as cards rather than the default list. Add `--html markdown`, `--html mixed`, `--html html` or `--html raw` to choose how page content is written ([below](#man-export-html)).
...
### <a id="man-export-combine"></a>Combining several archives

Several archives can go into **one** Hugo site, Jekyll site or Obsidian vault — a community captured in March and again in June, or two communities captured separately. Everything they hold sits side by side, in the same sections as a single export — except in a Hugo export of archives from more than one community, which is written [community first](#man-export-hugo): a folder per community, with its sections beneath it.

- **The most recent capture wins.** A wiki, blog, forum, library or Highlights area held by more than one archive appears once, and so does each page, post, topic and file. Each is taken from the archive that captured it most recently — item by item, so a wiki holds every page any of the archives captured, each in its newest version. A page's comments, a topic's replies and a page's attachments come with the copy that was kept; two copies' comments are never mixed. Where a page moved between captures, it sits under its newest parent.
- **Links between the archives become internal.** A link one archive could only point at the original deployment — because the page, post or file it names was in another archive — points inside the export once that archive is combined with it.
- **Nothing disappears silently.** The export counts the duplicates it merged and the links it made internal, and names, for every wiki, blog or forum held more than once, which archive's copy was kept. Every page records the archive it came from (`source_archive` in its front matter), and the Hugo and Obsidian `README.md` — for Jekyll a `sources.md` page — lists the archives that went in. The same identifier from two *different* deployments is two different things: both are kept, apart, and the export says so.

**From the console.** On the **Archives** screen, tick the archives and press **Export combined…** (with one archive ticked it is an ordinary **Export…**). The dialog takes you through four short steps, shown at its top, with **Back**, **Next** and **Close** always at its foot: **1 Archives** lists them — add or remove them there; **2 Format** chooses the format, with a line on what each is for, and how page content is written — for Hugo, also the starter site, and a line saying whether Hugo is on this computer (see [Preview with Hugo](#man-export-hugo-starter)); **3 Check** shows what the export will hold, which duplicates will be merged and whose copy wins, how many links will become internal, and the folder it will write beside the archives (`combined-…-hugo-content` and so on); **4 Export** writes it, showing the time it is taking, then says where it went and what to do next. Nothing is written until you press **Export**, and **Export** is only offered for choices that were checked: change an archive, the format or an option and it asks for a new **Check**.

**From the command line**, repeat `--archive` (or `--package`):

```
connections-export ingest --format hugo --archive PATH/TO/MARCH --archive PATH/TO/JUNE --output PATH/TO/CONTENT
```

### <a id="man-export-obsidian"></a>Obsidian — a linked vault

The Obsidian exporter converts a capture into a vault. Every wiki becomes one note per page, with the page hierarchy mirrored as nested folders. Every blog becomes a folder with one note per post, in the blog's own order, comments beneath each. Every forum becomes a folder with one note per topic, its replies laid out beneath it, each under its own heading and a reply to a reply indented one level further (as a quote), so a thread reads as a thread — each reply converted the same way as the topic, images and all, rather than flattened to a line. Links between items inside the export become `[[wikilinks]]` to the target note, whatever kind of item is on either end; other links are kept as-is. Images and attachments are copied out of the capture into the vault's `attachments/` folder and embedded by name (`![[file]]` / `[[file]]`) — an asset that was never captured becomes a visible marker instead of vanishing. YAML frontmatter carries each item's title, author, dates and tags, a `kind` for posts and topics, and its source provenance; a `README.md` at the vault root indexes every wiki, blog and forum. Open the folder in Obsidian as a vault and the whole community is there to browse.

### <a id="man-export-jekyll"></a>Jekyll — a publishable site

The Jekyll exporter writes a site fragment: dated Markdown files under `_posts/` — pages, blog posts, and forum topics with their replies beneath them, a reply to a reply indented one level further — with images under `assets/images/imported/`, ready to drop into a Jekyll site whose `_config.yml` and layouts you own. Each post names the `post` layout, which Jekyll's starter site and most themes provide, so a post shows its title and author out of the box; a site that uses another layout name can change it, or override it for these posts in its `_config.yml`. It is the shape a static-site generator expects, so `jekyll serve` previews it locally and a push to a [GitHub Pages](https://pages.github.com) repository publishes it. It is the right choice when the goal is a public, browsable website rather than a private notebook — the same captured content, aimed at a different destination.

### <a id="man-export-hugo"></a>Hugo — content for your own site

The Hugo exporter writes **content**: a `content/` folder, and a `README.md` beside it. Unless you ask for the [starter site](#man-export-hugo-starter), there are no layouts, no theme and no `hugo.toml` — those belong to your site. Copy or merge the `content/` folder into your site's own `content/` folder, at its root, and build.

Each wiki, blog, forum, file library and Highlights area becomes a section (`content/wikis/<wiki>/`, `content/blogs/<blog>/`, `content/forums/<forum>/`, `content/files/<library>/`, `content/highlights/<community>/`). A wiki's page tree becomes nested *page bundles*: a page with sub-pages is a folder with an `_index.md`, a page without one a folder with an `index.md`, and `weight` in the front matter keeps the wiki's own order. Blog posts, forum topics and Highlights pages are one folder each; a topic's replies appear beneath it, a reply to a reply indented one level further (as a quote), comments beneath a page. Every image and attachment is copied **into the folder of the page that shows it**, so Hugo publishes it next to that page; a library's files sit in the library's folder, listed on its page. Anything never captured shows as a visible marker.

**Several communities, community first.** When the export holds content from more than one community — a capture that took in a sub-community too, or archives of different communities [combined](#man-export-combine) — each community gets a folder of its own, `content/<community>/`, with the sections above beneath it (`content/<community>/wikis/<wiki>/` and so on). Its `_index.md` is the community's overview (`kind: community`), listing its sections and how much each holds; content in no community, such as a wiki captured on its own, goes into `content/other/`. Two communities with the same title get two folders, the second tagged with a short code from its id. With one community, or none, the sections sit directly under `content/` as described above. Nothing already in the output folder is deleted, so exporting into the same folder after the number of communities changed leaves the earlier layout's folders beside the new ones — use a fresh folder. The starter site follows either layout: with several communities its home page and header list the communities, and every breadcrumb starts with the community.
...
### <a id="man-export-hugo-starter"></a>Hugo starter site — viewing an export on its own

Without a Hugo site of your own, a folder of content has nothing to show it with. The **starter site** fills that gap: ask for it (`--starter-site` on the command line; in the console's export dialog, **Add a starter site** under Hugo, ticked by default) and the export also writes, **beside** `content/` and never inside it:

- `hugo.toml` — the site's configuration: its title, relative addresses so the built site works from any folder, the items' tags, how the front page is drawn (below), the Connections server the content came from, linked in every page's footer, and — only when the page content includes HTML — the switch Hugo needs to show that HTML, with a comment saying why.
- `layouts/` — a small set of templates: a page frame with the sections in its header and, in its footer, where the content came from, a home page listing every wiki, blog, forum, library and Highlights area with how much each holds (with several communities, every community), each wiki's page tree in the wiki's own order beside its pages, blog posts and forum topics newest first, the file libraries, each page with its author, date, tags, community and a link back to the original in Connections, and a page per tag.
- `static/css/site.css` — one plain stylesheet, light and dark, that loads nothing from anywhere else.

**The front page: a list or cards.** The front page — and, with several communities, each community's page — comes in two designs. **List**, the default, is plain and quick to read: under *Communities*, each community's name and how many items it holds, *Not in a community* last; on a community's page each section (Wikis, Blogs, Forums, Files, Highlights) with its wikis, blogs and so on listed beneath it, each with its count ("Engineering Handbook — 8 pages"). A long name simply wraps onto the next line and is always shown in full. **Cards** puts each community, or each section, in a box of its own. Choose with `--starter-layout list` or `--starter-layout cards` (with `--starter-site`), or in the console's export dialog with **Front page as**, right under **Add a starter site**. The choice is one line in `hugo.toml`, under `[params]`: `homeLayout = "list"`. Change it to `"cards"` (or back) and the site is drawn the other way the next time Hugo builds it — `hugo server` shows it at once; there is no need to export again.

Then, with [Hugo](https://gohugo.io/installation/) installed (0.146 or later), run `hugo server` in the export folder and open the address it prints; `hugo` on its own builds the site into `public/`. The starter site is **a starting point**, meant to be edited or replaced — change the templates, restyle it, add a theme. To put the content into a site you already have, `content/` alone is what goes in; leave the starter files behind. The export's `README.md` says the same. It works the same for a [combined export](#man-export-combine), where each page also names the archive it came from.

**Preview with Hugo.** When Hugo is installed on this computer, the console offers **Preview with Hugo** after a Hugo export with its starter site: it runs *your own* Hugo — `hugo server` in the export folder — and opens the site in a new tab. The preview is served by Hugo itself, on a separate local address (`127.0.0.1` and a port of its own), never through the console: exported pages are other people's content, and they are kept away from the console's own address. One preview runs at a time; **Stop preview** ends it, and so does closing the console. Whether Hugo is there is shown as soon as you choose Hugo in the dialog, before anything is written: its version when it is found; otherwise that it was not found on this computer's `PATH`, with a link to Hugo's installation guide. The console sees the `PATH` it was started with, so if you installed Hugo after starting the console, restart the console. If Hugo is found but does not answer `hugo version` — the first run of a newly installed program on Windows can be slow while antivirus checks it — the dialog says where it found Hugo; choose Hugo again a moment later to ask once more. This tool never downloads or installs Hugo, and Hugo remains an independent third-party application, as above. If Hugo cannot build the site, the dialog shows Hugo's own message.
...
A link is rewritten to point inside the archive when its target is _also in this export_. So capturing community A on its own leaves its links into B as dead references that only resolve while the deployment is reachable; capturing A and B **together** makes those same links resolve inside the archive, with no extra work. Exporting B separately and keeping both folders looks equivalent and is not — it gives you two archives that each know the other's content only as an outside reference. That is what the Reader and the PDF show, since they read one archive at a time. The Hugo, Jekyll and Obsidian exports can still [combine separate archives](#man-export-combine) afterwards, and there the links between them do resolve — but capturing together is what makes them resolve everywhere, the Reader and the PDF included.
