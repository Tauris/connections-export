...
Add `--starter-site` to a Hugo export to write the [starter site](#man-export-hugo-starter) beside its content. Add `--html markdown`, `--html mixed`, `--html html` or `--html raw` to choose how page content is written ([below](#man-export-html)).
...
Several archives can go into **one** Hugo site, Jekyll site or Obsidian vault — a community captured in March and again in June, or two communities captured separately. Everything they hold sits side by side, in the same sections as a single export.
...
- `hugo.toml` — the site's configuration: its title, relative addresses so the built site works from any folder, the items' tags, and — only when the page content includes HTML — the switch Hugo needs to show that HTML, with a comment saying why.
- `layouts/` — a small set of templates: a page frame with the sections in its header, a home page listing every wiki, blog, forum, library and Highlights area with how much each holds, each wiki's page tree in the wiki's own order beside its pages, blog posts and forum topics newest first, the file libraries, each page with its author, date, tags, community and a link back to the original in Connections, and a page per tag.
